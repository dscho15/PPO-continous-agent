...
class PPO:
    ...
    def __init__(
        ...
        normalizing_factor_for_rewards: float = 10,
    ):
        ...
        self.max_timesteps_per_episode = 1024  # timesteps per episode
        self.gamma = 0.99  # discount factor
        self.lam = 0.95
        self.n_epochs = 5  # number of epochs
        self.n_trajectories = 1000
        self.clip_eps = 0.2  # PPO clipping parameter
        self.nf_rewards = normalizing_factor_for_rewards
    ...
    def get_gae_return(
        ...
        for ep in range(len(batch_advantages)):
            ...
            for t in reversed(range(len(rewards))):
                ...
                delta = (rewards[t] + self.gamma * values[t + 1] - values[t] if t < len(rewards) - 1 else rewards[t] - values[t]).item()
    ...
    def rollout(
        ...
        batch_actions: list = []        # for actionsre
        ...
        while t < self.timesteps_per_batch:
            ...
            for _ in range(self.max_timesteps_per_episode):
                ...
                reward = reward / self.nf_rewards
                
                # Store the episodic rewards
                episodic_rewards.append(reward)
                episodic_values.append(values.flatten().tolist())
                
                batch_actions.append(action.flatten().tolist())
                batch_log_probs.append(log_prob.flatten().tolist())
        ...
        # Convert tensors
        batch_obs: torch.FloatTensor = torch.tensor(
            batch_obs, dtype=torch.float32
        )
        ...
        batch_discounted_returns: torch.FloatTensor = torch.cat(
            batch_values
        )
    ...
    def train(self):
        ...
        for k in tqdm(range(self.n_trajectories), desc="Training"):
            ...
            with torch.inference_mode():
                (b_observations, b_actions, b_log_probs, b_advantages, batch_values) = self.rollout()

            # Compute the advantage
            b_normalized_advantage_k = (b_advantages - b_advantages.mean()) / (b_advantages.std() + torch.finfo(torch.float32).eps)

            # Load dataset
            data = Dataset(b_observations, b_actions, b_log_probs, batch_values, b_normalized_advantage_k)
            ...
            for _ in range(self.n_epochs):
                ...
                for observations, actions, old_log_probs, values, n_advantages in dataloader:
                                        
                    # Compute new log probabilities and the ratio
                    pred_values, new_log_probs = self.evaluate(observations, actions)

                    # https://cs.stackexchange.com/questions/70518/why-do-we-use-the-log-in-gradient-based-reinforcement-algorithms
                    ratios = torch.exp(new_log_probs - old_log_probs)

                    # PPO clipped objective
                    surrogate1 = ratios * n_advantages.unsqueeze(1)
                    surrogate2 = (torch.clamp(ratios, 1 - self.clip_eps, 1 + self.clip_eps) * n_advantages.unsqueeze(1)
                    )
            ...
            print(f"Actor Loss: {np.mean(loss_policy_list)} Critic Loss: {np.mean(loss_critic_list)}")
    ...
    def test(self, save_path: str):
        ...
        self.env.close()
        frames[0].save(f"{save_path}/animation.gif", save_all=True, append_images=frames[1:], loop=0, duration=100)
